...
public class ViewRecordsReleaseActionTest extends TestCase {
	...
	public void testApproveRequest() {
		...
		assertTrue(action.processRequest(release, true));
		...
		assertFalse(action.processRequest(release, true));
	}
	...
	public void testDenyRequest() {
		...
		assertTrue(action.processRequest(release, false));
		//Make sure that the records release status is properly updated
		try {
			release = rrDAO.getRecordsReleaseByID(requestID);
		} catch (DBException e) {
		...
		assertFalse(action.processRequest(release, false));
	}
