...
public class PatientDAO {
	...
	/**
	 * Check that a Email actually exists for patients.
	 * 
	 * @param mid mid
	 * @return check
	 * @throws DBException
	 */
	public boolean checkEmailExists(final String email) throws DBException {
		Connection conn = null;
		PreparedStatement pstmt = null;
		try {
			conn = factory.getConnection();
			pstmt = conn.prepareStatement("SELECT * FROM patients WHERE email=?");
	...
	public void editPatient(PatientBean p, long hcpid) throws DBException {
		...
		try {
			...
			ps = conn.prepareStatement("UPDATE patients SET firstName=?,lastName=?,email=?,"
					...
					+ "AlternateName=?, DateOfDeactivation=?, messageFilter=? "
					+ "WHERE MID=?");
	...
	public void addHistory(long pid, long hcpid) throws DBException {
		...
		try {
			conn = factory.getConnection();
			ps = conn.prepareStatement("INSERT INTO historypatients SELECT null, CURDATE(), ?, " +
					"p.* FROM patients p WHERE p.mid=?");
			ps.setLong(1, hcpid);
			ps.setLong(2, pid);
			ps.executeUpdate();
			ps.close();
	...
	public List<PatientBean> getAllPrepatients() throws DBException {
		...
		try {
			conn = factory.getConnection();
			ps = conn.prepareStatement("SELECT patients.* FROM patients " +
			"INNER JOIN users WHERE users.mid=patients.mid AND users.role='prepatient'");
			ResultSet rs = ps.executeQuery();
			List<PatientBean> loadlist = patientLoader.loadList(rs);
			rs.close();
			ps.close();
	 ...
	 * @throws DBException
	 */
	public boolean changeRole(long prepatientMID, String role) throws DBException {
		Connection conn = null;
		PreparedStatement ps = null;
		try {
			conn = factory.getConnection();
	...
	/**
	* Deletes Specific Patient from the database
	*/
	public boolean deletePatient(long patientMID) throws DBException {
		Connection conn = null;
		PreparedStatement ps = null;
		try {
			conn = factory.getConnection();
			ps = conn.prepareStatement("DELETE users.*, patients.* FROM users, patients "
									+ "WHERE users.mid=patients.mid AND users.mid=?;");
			ps.setLong(1, patientMID);
	...
	/**
	* Get patient with mid
	* @param MID patient mid
	* @throws ITrustException
	* @throws SQLException
	* @throws DBException
	*/
	public PatientBean getPatientFromMID(long mid) throws SQLException, DBException{
		Connection conn = null;
		PreparedStatement pstring = null;
		try {
				conn = factory.getConnection();
				pstring = conn.prepareStatement("SELECT * FROM patients "+
				"WHERE MID = ?;");
				pstring.setLong(1, mid);
				final ResultSet results = pstring.executeQuery();
				final List<PatientBean> pList = patientLoader.loadList(results);
				results.close();
				pstring.close();
				return pList.get(0);
			} catch (SQLException e) {
