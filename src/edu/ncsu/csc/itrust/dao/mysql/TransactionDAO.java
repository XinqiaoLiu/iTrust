...
public class TransactionDAO {
	...
	/**
	 * Return a list of all transactions in which login user, secondary user transaction type meets the requirements, within the dates
	 * 
	 * @param loginUser The role of login user.
	 * @param secondaryUser The role of secondary user.
	 * @param startDate The starting date as a java.util.Date
	 * @param endDate The ending date as a java.util.Date
	 * @param transactionType The target transaction type
	 * @return A java.util.List of transactions.
	 * @throws DBException
	 */
	public List<TransactionBean> getFilteredLogs(String loginUser, String secondaryUser, Date startDate, Date endDate, int transactionType) throws DBException {
		Connection conn = null;
		PreparedStatement ps = null;
		try {
			conn = factory.getConnection();
			ps = conn.prepareStatement("SELECT a.* FROM (SELECT transactionlog.* FROM users INNER JOIN transactionlog " 
									+ "WHERE users.mid = transactionlog.loggedInMID AND (users.role=? OR ?='All roles')) as a "
									+ "INNER JOIN users WHERE users.mid = a.secondaryMID AND (users.role=? OR ?='All roles') "
									+ "AND a.timeLogged >= ? AND a.timeLogged <= ? "
									+ "AND (a.transactionCode=? OR ?=0) ORDER BY timeLogged DESC");
			
			ps.setString(1, loginUser);
			ps.setString(2, loginUser);
			ps.setString(3, secondaryUser);
			ps.setString(4, secondaryUser);
			ps.setTimestamp(5, new Timestamp(startDate.getTime()));
			ps.setTimestamp(6, new Timestamp(endDate.getTime() + 1000L * 60L * 60 * 24L));
			ps.setInt(7, transactionType);
			ps.setInt(8, transactionType);
