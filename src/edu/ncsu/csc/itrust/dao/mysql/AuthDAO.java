...
public class AuthDAO {
	...
	/**
	 * Returns the role of a particular user with MID
	 * 
	 ...
	 * @throws DBException
	 */
	public String getRoleFromMID(long mid) throws ITrustException, DBException {
		Connection conn = null;
		PreparedStatement ps = null;
		try {
			conn = factory.getConnection();
			...
			if (rs.next()) {
				...
				rs.close();
				ps.close();
				throw new ITrustException("User does not exist with the designated MID");
			}
		} catch (SQLException e) {
	...
	/**
	 * Returns the role of a particular patient
	 * 
