...
public class ApptDAO {
	...
	public void scheduleAppt(final ApptBean appt) throws SQLException, DBException {
		...
		pstring = conn.prepareStatement(
				"INSERT INTO appointment (appt_type, patient_id, doctor_id, sched_date, comment, rating) "
			  + "VALUES (?, ?, ?, ?, ?, ?)");
	...
	public void editAppt(final ApptBean appt) throws SQLException, DBException {
		...
		pstring = conn.prepareStatement(
				"UPDATE appointment SET appt_type=?, sched_date=?, comment=?, rating=? WHERE appt_id=?");
		pstring.setString(1, appt.getApptType());
		pstring.setTimestamp(2, appt.getDate());
		pstring.setString(3, appt.getComment());
		pstring.setInt(4, appt.getRating());
		pstring.setInt(5, appt.getApptID());
  ...
  /**
	 * Get appointments within numDays for reminder
   * @param numDays The num of days within which the appointment reminder should be sent.
	 * @throws ITrustException
	 * @throws SQLException
   * @throws DBException
	 */
	public List<ApptBean> getUpcomingAppointmentForReminder(int numDays) throws SQLException, DBException{

		Connection conn = null;
		PreparedStatement pstring = null;
		try {
				conn = factory.getConnection();	
				pstring = conn.prepareStatement("SELECT * FROM appointment "
				...
				Date futureD = calendar.getTime();
				Timestamp futureDate = new Timestamp(futureD.getTime());
				pstring.setTimestamp(1, futureDate);
				final ResultSet results = pstring.executeQuery();
				final List<ApptBean> pList = this.abloader.loadList(results);
				results.close();
				pstring.close();
				return pList;
			} catch (SQLException e) {
	...
	public double getHcpRating(long mid) throws DBException {
		Connection conn = null;
		PreparedStatement pstring = null;
		try{
			conn = factory.getConnection();
			pstring = conn.prepareStatement("SELECT COALESCE(AVG(rating), 0) as val" +  
											"  from appointment where doctor_id = ?;");
			pstring.setLong(1, mid);
			ResultSet results = pstring.executeQuery();
			double ret = 0;
			if(results.next()){
				ret = results.getDouble("val");
			} 
			results.close();
			pstring.close();
			return ret;
