...
public class ViewDiagnosisStatisticsAction {
	...
	public boolean isFluEpidemic(String curDateStr, String zip) throws ParseException, DBException {
		...
		double thresholdN = calcThreshold(weekOfYr-2); //IT'S PRECEDING TWO WEEKS
		...
		cal.add(Calendar.HOUR, -2*12*7); //IT'S PRECEDING TWO WEEKS
		...
		double threshold2weeksago = calcThreshold(weekOfYr - 2);
		double threshold1weekago = calcThreshold(weekOfYr - 1);

		cal.add(Calendar.HOUR, -24*7);
		DiagnosisStatisticsBean db1 = diagnosesDAO.getCountForWeekOf(ICD_INFLUENZA, zip, cal.getTime());
		cal.add(Calendar.HOUR, -24*7);
		DiagnosisStatisticsBean db2 = diagnosesDAO.getCountForWeekOf(ICD_INFLUENZA, zip, cal.getTime());

		double twoWeek = (double) db2.getRegionStats();
		double oneWeek = (double) db1.getRegionStats();

		return ((oneWeek > threshold1weekago) && (twoWeek > threshold2weeksago));
	}
	...
	public boolean isMalariaEpidemic(String weekDate, String zip, String thresholdStr) throws DBException, ParseException {
		...
		cal.add(Calendar.HOUR, -7*24); //make this negative
		...
		while( cal.getTime().before(wkDate) && cal.get(Calendar.YEAR) != wkDateCal.get(Calendar.YEAR)) {
			...
			cal.add(Calendar.HOUR, -7*24);//made this negative
