...
public class ViewMyRecordsAction {
	...
	private HashMap<String,String> diseaseICD = new HashMap<String, String>();
	...
	public ViewMyRecordsAction(DAOFactory factory, long loggedInMID) {
		...
		diseaseICD.put("Cancer","199");
		diseaseICD.put("Diabetes","250");
		diseaseICD.put("HeartDisease","402");
	}
	...
	/**
	 * Checks to see if family members have disease with the disease name
	 * 
	 * @param member the family member in question
	 * @param diseaseName the family member in question
	 * @return true if the family member has the risk factor, otherwise false.
	 * @throws ITrustException
	 */
	public boolean doesFamilyMemberHaveDisease(FamilyMemberBean member, String disease) throws ITrustException {
		List<DiagnosisBean> diagnoses = patientDAO.getDiagnoses(member.getMid());
		String code = diseaseICD.get(disease);
		if(diagnoses.size() == 0)
			return false;
		for(DiagnosisBean diag : diagnoses) {
			if(diag.getICDCode().startsWith(code))
				return true;
		}
		return false;
	}
